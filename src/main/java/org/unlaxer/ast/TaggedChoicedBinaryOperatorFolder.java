 ...
 *
 * Assumes choice wrappers are normalized beforehand:
 * - NormalizeChoicedOperandMapper
 * - NormalizeChoicedOperatorMapper
 *
 * Root token is mapped only when its parser is tagged with one of {@code rootKinds}.
 *
 * OneOrMore behavior:
 * - If the matched root kind is OneOrMoreChoicedOperatorOperandSuccessor, at least one fold must happen.
 */
public final class TaggedChoicedBinaryOperatorFolder implements ASTMapper {

  private final EnumSet<ASTNodeKind> rootKinds;

  public TaggedChoicedBinaryOperatorFolder(EnumSet<ASTNodeKind> rootKinds) {
  ...
  public TaggedChoicedBinaryOperatorFolder(ASTNodeKind... kinds) {
  ...
  public Token toAST(ASTMapperContext context, Token parsedToken) {
    ...
    if (matchedRoot == ASTNodeKind.OneOrMoreChoicedOperatorOperandSuccessor && foldedCount == 0) {
      return parsedToken;
    }
