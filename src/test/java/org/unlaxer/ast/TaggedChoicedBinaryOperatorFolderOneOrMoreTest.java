...
public class TaggedChoicedBinaryOperatorFolderOneOrMoreTest {
  ...
  static class DummyParser extends AbstractParser {
    ...
    @Override public void prepareChildren(Parsers childrenContainer) { /* no children */ }
  ...
  public void oneOrMore_returnsSame_whenOnlyOneOperand() {
    Parser rootP = new DummyParser(ASTNodeKind.OneOrMoreChoicedOperatorOperandSuccessor);
    Parser operandP = new DummyParser(ASTNodeKind.Operand);

    Token root = new Token(
        TokenKind.consumed,
        new TokenList(tok("1", operandP)),
        rootP
    );

    ASTMapperContext ctx = ASTMapperContext.create(
        new TaggedChoicedBinaryOperatorFolder(ASTNodeKind.OneOrMoreChoicedOperatorOperandSuccessor)
    );

    Token ast = ctx.toAST(root);
    assertSame(root, ast); // ★ここが ZeroOrMore と違う
  }
  ...
  public void oneOrMore_folds_whenHasAtLeastOneOperatorOperandPair() {
    ...
    Parser choiceRootP = new DummyParser(ASTNodeKind.ChoicedOperatorRoot);
    Parser opP = new DummyParser(ASTNodeKind.Operator);

    Token one = tok("1", operandP);
    Token plusReal = tok("+", opP);
    Token plusWrapped = new Token(TokenKind.consumed, new TokenList(plusReal), choiceRootP);
    Token two = tok("2", operandP);

    Token root = new Token(TokenKind.consumed, new TokenList(one, plusWrapped, two), rootP);
